...
import com.mojang.authlib.properties.Property;
import bep.hax.mixin.AccessorNbtCompound;
import bep.hax.mixin.IInteractEntityC2SPacket;
...
public class PacketNames {
    ...
    static {
        ...
        c2s(UpdateSelectedSlotC2SPacket.class, "PickFromInventory", packet -> "slot: " + packet.getSelectedSlot());
        c2s(PlayerActionC2SPacket.class, "PlayerAction", packet -> "action: " + packet.getAction().name() + "pos: " + packet.getPos().toShortString() + " direction: " + packet.getDirection().getName() + " sequence: " + packet.getSequence());c2s(PlayerInputC2SPacket.class, "PlayerInput", packet -> "forward: " + packet.input().forward() + " sideways: " + packet.input().right() +packet.input().left() + " jumping: " + packet.input().jump() + " sneaking: " + packet.input().sneak());
        ...
        c2s(PlayerInteractEntityC2SPacket.class, "PlayerInteractEntity", packet -> {
            try {
                IInteractEntityC2SPacket iPacket = (IInteractEntityC2SPacket) packet;

                // Try to get basic info
                String id = String.valueOf(iPacket.getId());

                // Try different ways to get type info
                Object typeHandler = iPacket.getType();
                String typeInfo = typeHandler.toString(); // This should give us some info
                String className = typeHandler.getClass().getSimpleName();

                return "id: " + id + " type: " + className + " (" + typeInfo + ") isPlayerSneaking: " + packet.isPlayerSneaking();
        ...
        s2c(LoginSuccessS2CPacket.class, "LoginSuccess", packet -> {
            StringBuilder builder = new StringBuilder("name: " + packet.profile().getName() +
                " id: " + packet.profile().getId().toString() +
                " properties: {");
        ...
        s2c(CooldownUpdateS2CPacket.class, "CooldownUpdate", packet -> "item: " + packet.cooldownGroup().getNamespace() + " cooldown: " + packet.cooldown());
        s2c(CraftFailedResponseS2CPacket.class, "CraftFailedResponse", packet -> "syncId: " + packet.syncId() + " recipeDisplay: " + packet.recipeDisplay());
        ...
        s2c(ExplosionS2CPacket.class, "Explosion", packet -> {
            StringBuilder builder = new StringBuilder("x: " + packet.center().getX() + " y: " + packet.center().getY() + " z: " + packet.center().getZ());
            packet.playerKnockback().ifPresent(vel -> {
                builder.append(" playerVelocityX: " + vel.getX() + " playerVelocityY: " + vel.getY() + " playerVelocityZ: " + vel.getZ());
            });
            // destructionType, radius, emitterParticle, and affectedBlocks removed in 1.21.4 mappings
            builder.append(" particle: " + packet.explosionParticle().getType().toString() + " soundEvent: ");
            builder.append(packet.explosionSound().value().id().toString());
            return builder.toString();
        });
        ...
        s2c(ParticleS2CPacket.class, "Particle", packet -> "count: " + packet.getCount() + " x: " + packet.getX() + " y: " + packet.getY() + " z: " + packet.getZ() + " offsetX: " + packet.getOffsetX() + " offsetY: " + packet.getOffsetY() + " offsetZ: " + packet.getOffsetZ() + " speed: " + packet.getSpeed() + " forceSpawn: " + packet.shouldForceSpawn() + " important: " + packet.isImportant() + " parameterType: " + packet.getParameters().getType());
        ...
        s2c(PlayerPositionLookS2CPacket.class, "PlayerPositionLook", packet -> {
            StringBuilder builder = new StringBuilder("teleportId: " + packet.teleportId() + " x: " + packet.change().position().getX() + " y: " + packet.change().position().getY() + " z: " + packet.change().position().getZ() + " yaw: " + packet.change().yaw() + " pitch: " + packet.change().pitch() + " flags: {");
            packet.relatives().forEach(flag -> builder.append("\n  ").append(flag.name()));
            builder.append("\n}");
            return builder.toString();
        });
        ...
        s2c(SynchronizeRecipesS2CPacket.class, "SynchronizeRecipes", packet -> {
            ...
            packet.stonecutterRecipes().entries().forEach(groupEntry -> {
                groupEntry.recipe().recipe().ifPresent(recipe -> builder.append("\n  id: ").append(recipe.id().toString()).append(" value: ").append(recipe.value().toString()));
            });
            builder.append("\n}");
            return builder.toString();
        });
